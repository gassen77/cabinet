...
import SessionBeans.CategorieproduitFacade;
import SessionBeans.FamilleFacade;
import SessionBeans.FournisseurFacade;
import SessionBeans.ProduitFacade;

import SessionBeans.UniteFacade;
...
public class ProduitController implements Serializable {
    ...
    @EJB
    private SessionBeans.ProduitFacade ejbFacade;
    private PaginationHelper pagination;
    private int selectedItemIndex;
  private String page;
      private Integer pageDestination;
      @EJB
    private FournisseurFacade ejbfournisseur; 
         @EJB
    private CategorieproduitFacade ejbcategorieproduit; 
             @EJB
    private FamilleFacade ejbfamille; 
                 @EJB
    private UniteFacade ejbunite; 
 ...
 FacesContext fc=FacesContext.getCurrentInstance();
        ...
        try {
            current = (Produit) map.get("current");
        } catch (Exception e) {
            current = null;
        }
                try{
     ...
     public void initPrepareCreate() {
          ...
          current.setDatemodification(new Date());
          current.setQteenstock(BigDecimal.ZERO.setScale(3, RoundingMode.UP));
        selectedItemIndex = -1;
    }
    ...
    public ProduitController() {
          FacesContext fc = FacesContext.getCurrentInstance();
          ejbfournisseur = (FournisseurFacade) fc.getApplication().getELResolver().getValue(fc.getELContext(), null, "FournisseurJpa");
          ejbunite = (UniteFacade) fc.getApplication().getELResolver().getValue(fc.getELContext(), null, "UniteJpa");
ejbfamille = (FamilleFacade) fc.getApplication().getELResolver().getValue(fc.getELContext(), null, "FamilleJpa");
ejbcategorieproduit = (CategorieproduitFacade) fc.getApplication().getELResolver().getValue(fc.getELContext(), null, "CategorieproduitJpa");
    ...
    private ProduitFacade getFacade() {
     ...
     public PaginationHelper getPagination() {
        if (pagination == null) {
            ...
            pagination = new PaginationHelper(10,"Select o from Produit o",s,o) {
    ...
    public String rechercherListItems(){
        String requette="Select o from Produit o where ";
    ...
    public String create() {
        ...
        try
        {
         l = getFacade().findByParameterSingleResultCountsansparam("Select max(c.id) from Produit c");
         if (l==null)
           {
            l = new Long(1);
           }
    ...
    public static class ProduitControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
           ...
           try{ ProduitController controller = (ProduitController) facesContext.getApplication().getELResolver().
      ...
      public List<Fournisseur> autocompleteFournisseur(String code) {
        ...
        try {
            result = (List<Fournisseur>) requeteFournisseur(code);
            return result;
        } catch (Exception d) {
    ...
    public List<Fournisseur> requeteFournisseur(String code) {
        String q = "Select b from Fournisseur b where b.code like :code or b.nom like :code";
        List<Fournisseur> l = this.ejbfournisseur.findByParameterAutocomplete(q, "code", code + "%", 50);
        return l;
    }
 /////////////unite/////////////////////
        public List<Unite> autocompleteUnite(String code) {
        List<Unite> result = new ArrayList<Unite>();
        try {
            result = (List<Unite>) requeteUnite(code);
            return result;
        } catch (Exception d) {
            result = new ArrayList<Unite>();
            return result;
        }
    }

    public List<Unite> requeteUnite(String code) {
        String q = "Select b from Unite b where b.code like :code or b.libelle like :code";
        List<Unite> l = this.ejbunite.findByParameterAutocomplete(q, "code", code + "%", 50);
        return l;
    }
    ////////////////////Famille//////////////////
         public List<Famille> autocompleteFamille(String code) {
        List<Famille> result = new ArrayList<Famille>();
        try {
            result = (List<Famille>) requetefamille(code);
            return result;
        } catch (Exception d) {
            result = new ArrayList<Famille>();
            return result;
        }
    }

    public List<Famille> requetefamille(String code) {
        String q = "Select b from Famille b where b.code like :code or b.libelle like :code";
        List<Famille> l = this.ejbfamille.findByParameterAutocomplete(q, "code", code + "%", 50);
        return l;
    }
     ////////////////////Famille//////////////////
         public List<Categorieproduit> autocompletecategorieproduit(String code) {
        List<Categorieproduit> result = new ArrayList<Categorieproduit>();
        try {
            result = (List<Categorieproduit>) requetecategorieproduit(code);
            return result;
        } catch (Exception d) {
            result = new ArrayList<Categorieproduit>();
            return result;
        }
    }

    public List<Categorieproduit> requetecategorieproduit(String code) {
        String q = "Select b from Categorieproduit b where b.code like :code or b.libelle like :code";
        List<Categorieproduit> l = this.ejbcategorieproduit.findByParameterAutocomplete(q, "code", code + "%", 50);
        return l;
    }
    ...
    public void handleFileUpload(FileUploadEvent event) {
        ...
        try {
          ...
          //  String nomImage = this.nomImage(event.getFile().getFileName());
           String nomImage="photo";
              File directory = new File(FacesContext.getCurrentInstance().getExternalContext().getRealPath("/pages/images/produits"));
            directory.mkdirs();
            System.out.println("nomImage "+nomImage);
            OutputStream output = new FileOutputStream(new File(FacesContext.getCurrentInstance().getExternalContext().getRealPath("/pages/images/produits") + "/" + nomImage + ".jpg"));
            int data = input.read();
            while (data != -1) {
                output.write(data);
                data = input.read();
            ...
            this.getSelected().setImage("/pages/images/produits/" + nomImage + ".jpg");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    protected static Semaphore semaphoreImage = new Semaphore(1, true);
     public String nomImage(String s) {
       ...
       public void impressionPdfProduit() throws JRException, IOException {
            ...
            try {
                ...
                jasperDesign.setName("ExtraitProduit");
                ...
                libelleTitre.setText("Liste des Produits");
...
//                jrFame.addElement(libelleSociete);
               ...
               query.setText("SELECT cabinet.`id` AS cabinet_id, cabinet.`code` AS cabinet_code, cabinet.`libelle` AS cabinet_libelle, cabinet.`adresse` AS cabinet_adresse, cabinet.`fax` AS cabinet_fax, cabinet.`tel` AS cabinet_tel, cabinet.`matricule` AS cabinet_matricule, cabinet.`ville` AS cabinet_ville, cabinet.`matriculefiscale` AS cabinet_matriculefiscale, cabinet.`codepostal` AS cabinet_codepostal, produit.`id` AS produit_id, produit.`unite` AS produit_unite, produit.`categorieproduit` AS produit_categorieproduit, produit.`fournisseur` AS produit_fournisseur, produit.`code` AS produit_code, produit.`libelle` AS produit_libelle, produit.`reference` AS produit_reference, produit.`qteenstock` AS produit_qteenstock, produit.`datecreation` AS produit_datecreation, produit.`datemodification` AS produit_datemodification, produit.`image` AS produit_image, produit.`commentaire` AS produit_commentaire, produit.`famille` AS produit_famille, produit.`prixachat` AS produit_prixachat, fournisseur.`id` AS fournisseur_id, fournisseur.`code` AS fournisseur_code, fournisseur.`nom` AS fournisseur_nom, categorieproduit.`id` AS categorieproduit_id, categorieproduit.`code` AS categorieproduit_code, categorieproduit.`libelle` AS categorieproduit_libelle FROM `produit` produit LEFT JOIN `fournisseur` fournisseur ON fournisseur.`id` = produit.`fournisseur` LEFT JOIN `categorieproduit` categorieproduit ON produit.`categorieproduit` = categorieproduit.`id`, `cabinet` cabinet");
               System.out.println("Query" + query.getText());
                jasperDesign.setQuery(query);
                jasperDesign.setName("ListeProduitssrapport");
                jasperDesign.setLeftMargin(10);
                jasperDesign.setRightMargin(10);
                jasperDesign.setTopMargin(10);
                jasperDesign.setBottomMargin(10);
...

                {
                    ...
                    styleLibelleColumnHeader(jasperDesign, bandHeader, "produit_code", "(($F{produit_code}!=null)&($F{produit_code}.equals(\"\")==false))?$F{produit_code}:Character.toString(' ')", libelleColonneCode, "CODE", 0, 60, bandSomme);
...

                {
                    ...
                    styleLibelleColumnHeader(jasperDesign, bandHeader, "produit_libelle", "(($F{produit_libelle}!=null)&($F{produit_libelle}.equals(\"\")==false))?$F{produit_libelle}:Character.toString(' ')", libelleColonneDesignation, "LIBELLE", 0, 200, bandSomme);
...
//                }
 ...
 ((JRDesignSection) jasperDesign.getDetailSection()).addBand(bandHeader);
                ...
                String nomFichier = "RapportListeClient.pdf";
